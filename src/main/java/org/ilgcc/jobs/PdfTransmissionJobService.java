...
public class PdfTransmissionJobService {
    ...
    public void enqueuePdfTransmissionJob(String objectKey) throws IOException {
        String presignedUrl = generatePresignedUrl(objectKey);
        log.info("Generated presigned URL: {} for object: {}", presignedUrl, objectKey);
        sendDocumentTransferServiceRequest(presignedUrl);
    }
    ...
    private void sendDocumentTransferServiceRequest(String presignedUrl) throws IOException {
        // Send request to document transfer service
        URLConnection httpUrlConnection = documentTransferServiceUrl.openConnection();
        ...
        try (OutputStream os = httpUrlConnection.getOutputStream()) {
            ...
            log.error("There was en error when sending the request to the document transfer service", e);
        }
