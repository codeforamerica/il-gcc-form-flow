...
@Component
@Slf4j
public class SendFamilyConfirmationEmail extends CreateEmailMessage {

    public static String EMAIL_SENT_STATUS_INPUT_NAME = "parentContactEmail";

    public static String RECIPIENT_EMAIL_INPUT_NAME = "providerResponseContactEmail";

    public SendFamilyConfirmationEmail(SendEmailJob sendEmailJob, SubmissionRepositoryService submissionRepositoryService) {
        super(sendEmailJob, submissionRepositoryService, EmailType.FAMILY_CONFIRMATION_EMAIL);
    }
    ...
    protected Content emailBody() {
        String parentFirstName = currentSubmission.getInputData().get("parentFirstName").toString();
        String emailLink = currentSubmission.getInputData().get("emailLink").toString();
        String ccrAndR = currentSubmission.getInputData().get("ccrrName").toString();
        String submittedDate = SubmissionUtilities.getFormattedSubmittedAtDate(currentSubmission);
        ...
        String p5 = messageSource.getMessage("email.family-confirmation.p5",
                new Object[]{currentSubmission.getShortCode(), submittedDate},
                locale);
