...
@Getter
@Setter
public class AllProvidersRespondedFamilyConfirmationEmail {

    private Map<String, Object> emailData;
    private MessageSource messageSource;
    private Locale locale;

    public AllProvidersRespondedFamilyConfirmationEmail(Map<String, Object> emailData, MessageSource messageSource, Locale locale) {
   ...
   public ILGCCEmailTemplate createTemplate(){
    ...
    private String setBodyCopy(Map<String, Object> emailData) {
        ...
        return p1 + p2 + allProviderResponses + p4 + p5 + p6;
    }
  private String getAllProviderResponses(Map<String, Object> emailData, MessageSource messageSource, Locale locale) {
      List<Map<String, Object>> providers = (List<Map<String, Object>>) Optional.ofNullable(emailData.get("providers")).orElse(Collections.emptyList());
      ...
      for (Map<String, Object> provider : providers) {
        if (provider.getOrDefault("providerResponseAgreeToCare", "").equals("true")){
          providerResponses = String.format("%s %s", providerResponses,
              messageSource.getMessage("email.all-providers-responded-family-confirmation-email.li-agreed-to-care", new Object[]{provider.get("providerResponseName")}, locale));
        } else if (provider.getOrDefault("providerResponseAgreeToCare", "").equals("false")){
          providerResponses = String.format("%s %s", providerResponses,
              messageSource.getMessage("email.all-providers-responded-family-confirmation-email.li-did-not-agree-to-care", new Object[]{provider.get("providerResponseName")}, locale));
        } else{
          providerResponses = String.format("%s %s", providerResponses,
